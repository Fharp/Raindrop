...
MASTER_DB_FLOOR = -4.0      # intensity=0 时的雨床增益
MASTER_DB_LULL = -6.0       # 事件内被跨接的那个无雨小时
# 这两个值刻意压得很浅。下雨期间白噪音不该断，小雨的「小」由素材音色和
# EQ 倾斜去表达，不靠拉低音量——音量一掉，听感就是雨停了而不是雨变小了。
# 四条雨层是等功率归一的，所以主增益是唯一会改变整体响度的量。
...
def render_frame(idx, row, ev_burst, city_anchors, cfg, tz, slug):
    ...
    if lull:
        inten = 0.0
        master_db = cfg.lull_db
    else:
        inten = intensity_of(row["liq"], city_anchors, cfg.calibration)
        master_db = cfg.floor_db * ((1.0 - inten) ** MASTER_CURVE)
...
def main():
    ...
    ap.add_argument("--floor-db", type=float, default=MASTER_DB_FLOOR,
                    help="intensity=0 时的主增益 dB，默认 -4；压得越深小雨越轻")
    ap.add_argument("--lull-db", type=float, default=MASTER_DB_LULL,
                    help="事件内被跨接的无雨小时的主增益 dB，默认 -6")
    ...
    cfg.floor_db, cfg.lull_db = args.floor_db, args.lull_db
